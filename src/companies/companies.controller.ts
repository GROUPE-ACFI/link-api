...
import { ApiBearerAuth, ApiCreatedResponse, ApiOkResponse, ApiTags, ApiParam } from '@nestjs/swagger';
...
export class CompaniesController {
  ...
  async findAll(@Query() query: QueryCompanyDto): Promise<InfinityPaginationResponseDto<Company>> {
    const page = query.page ?? 1;
    let limit = query.limit ?? 10;
    if (limit > 50) limit = 50;
    return infinityPagination(
      await this.service.findManyWithPagination({ paginationOptions: { page, limit } }),
      { page, limit },
    );
  }
  ...
  @ApiOkResponse({ type: Company })
  @ApiParam({ name: 'id', type: String, required: true })
  update(@Param('id') id: Company['id'], @Body() dto: UpdateCompanyDto): Promise<Company | null> {
