...
import { OpportunityMapper } from '@opportunities/infrastructure/persistence/relational/mappers/opportunity.mapper';
import { QueryOpportunityDto } from '@opportunities/dto/query-opportunity.dto';

@Injectable()
export class OpportunitiesRelationalRepository
  ...
  async create(data: Omit<Opportunity, 'id'>): Promise<Opportunity> {
    const entity = await this.repository.save(
      this.repository.create(
        OpportunityMapper.toPersistence(data as Opportunity),
      ),
    );
